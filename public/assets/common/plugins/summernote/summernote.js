...
(function (factory) {
    ...
    var Editor = function () {
        ...
        this.insertImage = function ($editable, sUrl) {
            async.createImage(sUrl).then(function ($image) {
                ...
                $image.css({
                    display: 'block',
                    width: Math.min($editable.width(), $image.width()) === $editable.width() ? '100%' : $image.width()
        ...
        this.insertVideo = function ($editable, sUrl) {
            ...
            var $video = $('<video>')
                .attr('src', sUrl).attr("controls", "controls");
    ...
    var Dialog = function () {
         ...
         * @return {Promise}
         */
        this.showImageDialog = function ($editable, $dialog, options) {
            return $.Deferred(function (deferred) {
                var $imageDialog = $dialog.find('.note-image-dialog');
                if (options.imageUpload.url != null) {
                    var btnUpload = $('#' + options.imageUpload.btnId);
                    try {
                        new AjaxUpload(btnUpload, {
                            action: options.imageUpload.url,
                            name: options.imageUpload.name,
                            // data: {'groupname' : options.url},
                            onComplete: function (file, response) {
                                var res = JSON.parse(response);
                                $imageDialog.modal('hide');
                                deferred.resolve(res.fileurl + '/' + res.filename);
                            }
                        });
                    } catch (e) {
                        alert(e);
    ...
    var EventHandler = function () {
         ...
         * @param {File[]} files
         */
        var insertImages = function ($editable, files, options) {
        ...
        var hToolbarAndPopoverClick = function (event) {
            ...
            if ($btn.length > 0) {
                ...
                if ($.inArray(sEvent, ['backColor', 'foreColor']) !== -1) {
                    ...
                    dialog.showImageDialog($editable, $dialog, options).then(function (data) {
                        if (typeof data === 'string') {
                            editor.restoreRange($editable);
                            editor.insertImage($editable, data, options);
                        } else {
                            insertImages($editable, data, options);
                        }
                    });
        ...
        var attachDragAndDropEvent = function (oLayoutInfo) {
            ...
            $dropzone.on('drop', function (event) {
                ...
                if (dataTransfer && dataTransfer.files) {
                    ...
                    insertImages(oLayoutInfo.editable(), dataTransfer.files, oLayoutInfo.editor().data('options'));
                }
                event.preventDefault();
        ...
        var tplDialogs = function (lang, options) {
            var tplImageDialog = function () {
                var body = '<h5>' + lang.image.selectFromFiles + '</h5>' +
                    (options.imageUpload.url == null ? '<input class="note-image-input" type="file" name="files" accept="image/*" />' :
                        '<div id="PageButtonWrapper"><input type="button" class="btn btn-primary" id="' + options.imageUpload.btnId + '" value="Select image"/></div>') +
